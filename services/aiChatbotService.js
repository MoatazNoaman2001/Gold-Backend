...
export async function getChatbotResponse(message, user) {
  ...
  // Detect budget queries (e.g., "I have 5000 EGP and want a ring" or "bracelets above 3500")
  const budgetPattern = /(budget|ميزانية|have|عندي|with|بـ|for|بـسعر|بسعر|بميزانية|بمبلغ|amount|مبلغ|price|سعر)[^\d]*(\d{2,})([^\d]+)?(ring|خاتم|bracelet|سوار|bangle|اسورة|necklace|قلادة|عقد|bridal|طقم|set|طقم عروس|bride|عروس)?/i;
  const abovePattern = /(above|over|greater than|more than|أكثر من|فوق|أعلى من)[^\d]*(\d{2,})([^\d]+)?(ring|خاتم|bracelet|سوار|bangle|اسورة|necklace|قلادة|عقد|bridal|طقم|set|طقم عروس|bride|عروس)?/i;
  let match = message.match(budgetPattern);
  let aboveMatch = message.match(abovePattern);
